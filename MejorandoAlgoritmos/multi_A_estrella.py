...
def resolverCamino(almacen,inicio,posiciones_estanterias,final=None):
    ...
    est_posiciones = posiciones_estanterias

    # Primer trayectoria
    algoritmo = A_estrella(start_pos,est_posiciones[0],2,almacen.limits,almacen.obstaculos)
    ...
    for i in range(1,len(est_posiciones)):
        # Segunda trayectoria
        algoritmo = A_estrella(qfinal,est_posiciones[i],2,almacen.limits,almacen.obstaculos)
        algoritmo.vecinos_type = "SIMPLES" #"COMPLETOS"(en diagonales)/"SIMPLES"(mov. rectos)
        solucion,qfinal,g,h = algoritmo.start()
        distTotal+=g
        caminoTotal.append(solucion)
    ...
    # Ultima trayectoria
    algoritmo = A_estrella(qfinal,end_pos,2,almacen.limits,almacen.obstaculos)
    algoritmo.vecinos_type = "SIMPLES" #"COMPLETOS"(en diagonales)/"SIMPLES"(mov. rectos)
    solucion,qfinal,g,h = algoritmo.start()
    distTotal+=g
    caminoTotal.append(solucion)
