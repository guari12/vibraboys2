    ...
    def constraint(self):
        ...
        for i in range(len(self.tareas)):
            for j in range(len(self.tareas)):
                alcance = [self.tareas[i]["id"],self.tareas[j]["id"]] # crear key
                relacion = []
                for A in self.D[i]["Dominio"]:
                    for B in self.D[j]["Dominio"]:
                        combinacion = [A,B] # (m1,1),(m1,2)
                        if A["M"]==B["M"]:
                            # los tiempos no se tienen que pisar
                            # si se pisan 
                            if A["S"]+self.tareas[i]["D"]<=B["S"]:
